...
async def process_video():
    try:
        ...
        long_transct, video_info = await get_llm_transcript(videoUrl)
        ...
        transct = await get_youtube_transcript(videoUrl)
        logging.debug(f'Process transcript for summary')
        try:
            summaryv2 = await openAI_summary(long_transct, api_key)
        except:
            summaryv2 = 'An error has occurred: This video is too long, please choose a shorter video. If you want to summarize a larger video, please contact the administrator through the "Contact Us" section.'
